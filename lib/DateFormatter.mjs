import { dayCount } from './days.mjs'
import { tokenize } from './tokenizer.mjs'
import { generateOffset, generateOffsetColon } from './offset.mjs'
import { Generator } from './formatterGenerator.mjs'
import { localeEN } from './locales/en.mjs'
import { localeDE } from './locales/de.mjs'
import { localeIT } from './locales/it.mjs'
...
export class DateFormatter {
  ...
  format (now) {
    return this.#formatter(now, dayCount)
  }
  ...
  setLocale (language) {
    this.months = this.#locales[language].months
    this.weekdays = this.#locales[language].weekdays
    this.monthsShort = this.#locales[language].monthsShort
    this.weekdaysShort = this.#locales[language].weekdaysShort
  }
...
function buildFormatter (dateFormat, options) {
  ...
  var tokens = tokenize(dateFormat)
